...
import time
from parallel_ta_agem import ParallelTrainingManager, create_parallel_params, run_parallel_ta_agem
...
ENABLE_PARALLEL = True  # False to use sequential training
...
if ENABLE_PARALLEL:
    ...
    print(f"Using parallel training with strategy: {PARALLEL_STRATEGY}")
    parallel_params = create_parallel_params(params, PARALLEL_STRATEGY, NUM_PROCESSES)
    run_parallel_ta_agem(
        parallel_params, model, optimizer, criterion, clustering_memory,
    ...
    for task_id, task_dataloader in enumerate(task_dataloaders):
        ...
        # Evaluation and metrics (your existing code)
        model.eval()
