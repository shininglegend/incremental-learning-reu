...
def train_single_task_parallel(args):
    ...
    for epoch in range(params['num_epochs']):
        model.train()
        epoch_losses = []
        # Initialize an accumulator for projected gradients for this epoch
        accumulated_projected_grads = torch.zeros(0).to(DEVICE)  # Will concatenate flattened gradients
        first_batch_processed = False  # To check if we got gradients for initialization size

        with ThreadPoolExecutor(max_workers=4) as executor:
            batch_futures = []

            # CORRECTED: Iterate directly over task_dataloader for each epoch
            for batch_idx, (data, labels) in enumerate(task_dataloader):  # <--- Fix for Error 2
                data = data.to(DEVICE)
                labels = labels.to(DEVICE)

                future = executor.submit(
                    process_batch_parallel,
                    agem_handler, data, labels, local_memory  # Model is handled by agem_handler internally
                )
                batch_futures.append(future)

            # Collect results and accumulate projected gradients
            for future in as_completed(batch_futures):
                ...
                if batch_result['success']:
                    ...
                    if batch_result['projected_gradients_flat'] is not None:
                        # Initialize accumulated_projected_grads size on first successful gradient
                        if not first_batch_processed:
                            accumulated_projected_grads = torch.zeros_like(batch_result['projected_gradients_flat']).to(
                                DEVICE)
                            first_batch_processed = True

                        # Accumulate the projected gradients
                        accumulated_projected_grads += batch_result['projected_gradients_flat']

                    # Add samples to memory
                    for i in range(len(batch_result['data'])):
                        local_memory.add_sample(
                            batch_result['data'][i].to('cpu'),
                            batch_result['labels'][i].to('cpu')
                        )
                else:
                    print(f"Warning: Batch processing failed: {batch_result['error']}")

        # After all batches for the epoch are processed and gradients accumulated:
        if first_batch_processed:  # Only apply if at least one batch contributed gradients
            agem_handler.apply_accumulated_gradients(accumulated_projected_grads)
        else:
            print(f"Task {task_id}, Epoch {epoch + 1}: No gradients accumulated, skipping optimizer step.")
...
# In your main training file
def process_batch_parallel(agem_handler, data, labels, memory): # Removed 'model' arg, as agem_handler holds it
    """Process a single batch in parallel, computing and projecting gradients."""
    try:
        # Get memory samples for gradient projection.
        # Ensure memory samples are on the correct device for AGEMHandler's internal computations.
        # This line assumes memory.get_memory_samples() returns CPU tensors.
        memory_samples_on_device = [(s.to(agem_handler.device), l.to(agem_handler.device))
                                     for s, l in memory.get_memory_samples()]

        # Compute projected gradients and batch loss.
        # This method DOES NOT call optimizer.step()
        ...
        return {
            'loss': batch_loss,
            'projected_gradients_flat': projected_grads_flat, # Return the flattened projected gradients
