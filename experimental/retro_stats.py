...
def load_pickle_files(directory="test_results"):
    ...
    pickle_files = glob.glob(os.path.join(directory, "test_results_*.pkl"))
    ...
    for file_path in pickle_files:
        try:
            ...
            final_accuracy = None
            if 'overall_accuracy' in data and data['overall_accuracy']:
                final_accuracy = data['overall_accuracy']
...
def create_summary_table(statistics):
    ...
    display_df['Mean Accuracy'] = display_df['Mean Accuracy'].apply(lambda x: f"{x:.4f}")
    display_df['Std Deviation'] = display_df['Std Deviation'].apply(lambda x: f"{x:.4f}")
    display_df['99% CI Lower'] = display_df['99% CI Lower'].apply(lambda x: f"{x:.4f}" if not pd.isna(x) else "N/A")
    display_df['99% CI Upper'] = display_df['99% CI Upper'].apply(lambda x: f"{x:.4f}" if not pd.isna(x) else "N/A")
...
def main():
    ...
    # Save summary table
    summary_filename = f"{directory}/results_{timestamp}.csv"
    ...
    detailed_filename = f"{directory}/results_{timestamp}_detailed.csv"
